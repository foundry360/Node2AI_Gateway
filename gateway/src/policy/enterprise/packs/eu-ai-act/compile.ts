...
export type CompiledEuAiActPack = {
  pack_id: string;
  pack_version: string;
  rules: EuAiActCompiledRule[];
...
function packRootCandidates(): string[] {
  ...
  return [
    join(process.cwd(), 'policy-packs', 'eu-ai-act'),
    join(here, '../../../../../policy-packs/eu-ai-act'),
    join(here, '../../../../../../policy-packs/eu-ai-act'),
  ];
}
...
export function loadEuAiActPackSources(): {
  root: string | null;
  rules: EuAiActCompiledRule[];
  provenance: PackProvenanceGraph;
} {
  for (const root of packRootCandidates()) {
    ...
    try {
      ...
      let provenance: PackProvenanceGraph = structuredClone(EU_AI_ACT_PROVENANCE_GRAPH);
  ...
  return {
    ...
    provenance: EU_AI_ACT_PROVENANCE_GRAPH,
  };
}

export function compileEuAiActPack(): CompiledEuAiActPack {
  const loaded = loadEuAiActPackSources();
  const policies: PackPolicyMeta[] = [
    {
      ...
      description:
        'Applies Regulation (EU) 2024/1689 (EU AI Act) as regulatory obligation gates. Legal authority — does not certify organizational compliance.',
      owner: 'compliance',
      priority: 160,
      scope_tier: 'regulatory',
      domain: 'ai_risk',
      content_hash: 'sha256:eu_ai_act_pack_v1',
      pack_name: 'EU AI Act',
      pack_version: EU_AI_ACT_PACK_META.pack_version,
    },
    {
      ...
      description:
        'Adds complementary governance logging for EU AI Act-scoped responses (Art. 12 record-keeping family).',
      owner: 'compliance',
      priority: 160,
      scope_tier: 'regulatory',
      domain: 'ai_risk',
      content_hash: 'sha256:eu_ai_act_pack_v1_output',
      pack_name: 'EU AI Act',
      pack_version: EU_AI_ACT_PACK_META.pack_version,
    },
  ];
