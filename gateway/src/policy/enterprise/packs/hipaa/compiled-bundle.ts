...
export const HIPAA_RULES: HipaaCompiledRule[] = [
  ...
  {
    rule_id: 'HIPAA-R-INPUT-WRITE-REQUIRE-APPROVAL',
    name: 'Require approval for PHI write without prior authorization path',
    phase: 'input',
    priority: 90,
    requirement_type: 'DERIVED_CONTROL',
    conditions: {
    ...
    decision: 'REVIEW',
    reason_codes: ['HIPAA_PHI_WRITE_REQUIRES_APPROVAL'],
    obligation_ids: ['HIPAA-OBL-DISCLOSURE', 'HIPAA-OBL-INTEGRITY', 'HIPAA-OBL-AUTHORIZATION'],
    control_ids: ['ctrl_no_write_back', 'ctrl_audit_logging', 'ctrl_authorization'],
    enigma_obligations: [
    ...
    source_type: 'DERIVED_CONTROL',
    derived_from: ['src_45cfr164'],
    note: 'WRITE is not an automatic DENY. High-risk PHI write requires human approval; low-risk non-PHI writes are decided by other packs/baseline.',
  },
  {
    rule_id: 'HIPAA-R-INPUT-EXTERNALIZE-DENY',
    name: 'Deny PHI export/share/transmit without authorization path',
    phase: 'input',
    priority: 90,
    requirement_type: 'DERIVED_CONTROL',
    conditions: {
      regulatory_applicability: 'HIPAA',
      classification: 'PHI',
      operation_in: ['export', 'share', 'transmit'],
    },
    decision: 'DENY',
  ...
  },
  {
    ...
    note: 'TOKENIZE is an Enigma implementation_option, not a regulatory mandate. NO_EXTERNAL_TRANSMISSION means no raw PHI egress.',
  },
  {
    rule_id: 'HIPAA-R-INPUT-EXTERNAL-TOKENIZE',
    name: 'Enigma TOKENIZE when PHI external processing controls are satisfied',
    phase: 'input',
    priority: 55,
    requirement_type: 'IMPLEMENTATION_OPTION',
    conditions: {
    ...
    reason_codes: [
      ...
      'HIPAA_PHI_EXTERNAL_CONTROLS_SATISFIED',
    ],
    obligation_ids: ['HIPAA-OBL-MINIMUM-NECESSARY', 'HIPAA-OBL-PHI-PROTECTION', 'HIPAA-OBL-TRANSMISSION'],
    control_ids: ['ctrl_tokenization', 'ctrl_transmission_security', 'ctrl_audit_logging'],
    enigma_obligations: [
      'TOKENIZE_PII',
      'NO_EXTERNAL_TRANSMISSION',
      'LOG_GOVERNANCE_EVENT',
    ...
    sources: ['src_45cfr164'],
    source_type: 'IMPLEMENTATION_OPTION',
    note: 'Controlled external path: TOKENIZE is Enigma control; does NOT assert HIPAA compliance. Does not add LOCAL_MODEL_ONLY.',
  },
  {
