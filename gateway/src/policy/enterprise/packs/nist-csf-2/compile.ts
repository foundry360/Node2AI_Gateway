...
export type CompiledNistCsf2Pack = {
  pack_id: string;
  pack_version: string;
  rules: NistCsf2CompiledRule[];
...
function packRootCandidates(): string[] {
  ...
  return [
    join(process.cwd(), 'policy-packs', 'nist-csf-2'),
    join(here, '../../../../../policy-packs/nist-csf-2'),
    join(here, '../../../../../../policy-packs/nist-csf-2'),
  ];
}
...
export function loadNistCsf2PackSources(): {
  root: string | null;
  rules: NistCsf2CompiledRule[];
  provenance: PackProvenanceGraph;
} {
  for (const root of packRootCandidates()) {
    ...
    try {
      ...
      let provenance: PackProvenanceGraph = structuredClone(NIST_CSF_2_PROVENANCE_GRAPH);
  ...
  return {
    ...
    provenance: NIST_CSF_2_PROVENANCE_GRAPH,
  };
}

export function compileNistCsf2Pack(): CompiledNistCsf2Pack {
  const loaded = loadNistCsf2PackSources();
  const policies: PackPolicyMeta[] = [
    {
      ...
      description:
        'Applies NIST CSF 2.0 as Enigma cybersecurity governance evidence gates. Framework — does not certify NIST CSF compliance or produce a cybersecurity score.',
      owner: 'compliance',
      priority: 110,
      scope_tier: 'standard',
      domain: 'ai_risk',
      content_hash: 'sha256:nist_csf_2_pack_v1',
      pack_name: 'NIST CSF 2.0',
      pack_version: NIST_CSF_2_PACK_META.pack_version,
    },
    {
      ...
      description:
        'Adds complementary Enigma logging for NIST CSF 2.0-scoped responses (DETECT monitoring family).',
      owner: 'compliance',
      priority: 110,
      scope_tier: 'standard',
      domain: 'ai_risk',
      content_hash: 'sha256:nist_csf_2_pack_v1_output',
      pack_name: 'NIST CSF 2.0',
      pack_version: NIST_CSF_2_PACK_META.pack_version,
    },
  ];
