...
export type CompiledIso42005Pack = {
  pack_id: string;
  pack_version: string;
  rules: Iso42005CompiledRule[];
...
function packRootCandidates(): string[] {
  ...
  return [
    join(process.cwd(), 'policy-packs', 'iso-42005'),
    join(here, '../../../../../policy-packs/iso-42005'),
    join(here, '../../../../../../policy-packs/iso-42005'),
  ];
}
...
export function loadIso42005PackSources(): {
  root: string | null;
  rules: Iso42005CompiledRule[];
  provenance: PackProvenanceGraph;
} {
  for (const root of packRootCandidates()) {
    ...
    try {
      ...
      let provenance: PackProvenanceGraph = structuredClone(ISO_42005_PROVENANCE_GRAPH);
  ...
  return {
    ...
    provenance: ISO_42005_PROVENANCE_GRAPH,
  };
}

export function compileIso42005Pack(): CompiledIso42005Pack {
  const loaded = loadIso42005PackSources();
  const policies: PackPolicyMeta[] = [
    {
      ...
      description:
        'Applies ISO/IEC 42005:2025 as Enigma AI system impact-assessment evidence gates. Standard — does not certify ISO compliance or produce an impact score.',
      owner: 'compliance',
      priority: 120,
      scope_tier: 'standard',
      domain: 'ai_risk',
      content_hash: 'sha256:iso_42005_pack_v1',
      pack_name: 'ISO/IEC 42005',
      pack_version: ISO_42005_PACK_META.pack_version,
    },
    {
      ...
      description:
        'Adds complementary Enigma logging for ISO 42005-scoped responses (impact monitoring family).',
      owner: 'compliance',
      priority: 120,
      scope_tier: 'standard',
      domain: 'ai_risk',
      content_hash: 'sha256:iso_42005_pack_v1_output',
      pack_name: 'ISO/IEC 42005',
      pack_version: ISO_42005_PACK_META.pack_version,
    },
  ];
