...
export type CompiledNistPrivacyFrameworkPack = {
  pack_id: string;
  pack_version: string;
  rules: NistPrivacyFrameworkCompiledRule[];
...
function packRootCandidates(): string[] {
  ...
  return [
    join(process.cwd(), 'policy-packs', 'nist-privacy-framework'),
    join(here, '../../../../../policy-packs/nist-privacy-framework'),
    join(here, '../../../../../../policy-packs/nist-privacy-framework'),
  ];
}
...
export function loadNistPrivacyFrameworkPackSources(): {
  root: string | null;
  rules: NistPrivacyFrameworkCompiledRule[];
  provenance: PackProvenanceGraph;
} {
  for (const root of packRootCandidates()) {
    ...
    try {
      ...
      let provenance: PackProvenanceGraph = structuredClone(NIST_PRIVACY_FRAMEWORK_PROVENANCE_GRAPH);
  ...
  return {
    ...
    provenance: NIST_PRIVACY_FRAMEWORK_PROVENANCE_GRAPH,
  };
}

export function compileNistPrivacyFrameworkPack(): CompiledNistPrivacyFrameworkPack {
  const loaded = loadNistPrivacyFrameworkPackSources();
  const policies: PackPolicyMeta[] = [
    {
      ...
      description:
        'Applies NIST Privacy Framework 1.0 (CSWP 10) as Enigma privacy-risk governance evidence gates under privacy.nist_pf. Framework — does not certify NIST compliance or produce a privacy score.',
      owner: 'compliance',
      priority: 110,
      scope_tier: 'standard',
      domain: 'ai_risk',
      content_hash: 'sha256:nist_privacy_framework_pack_v1',
      pack_name: 'NIST Privacy Framework',
      pack_version: NIST_PRIVACY_FRAMEWORK_PACK_META.pack_version,
    },
    {
      ...
      description:
        'Adds complementary Enigma logging for NIST Privacy Framework-scoped responses (privacy-risk communication family).',
      owner: 'compliance',
      priority: 110,
      scope_tier: 'standard',
      domain: 'ai_risk',
      content_hash: 'sha256:nist_privacy_framework_pack_v1_output',
      pack_name: 'NIST Privacy Framework',
      pack_version: NIST_PRIVACY_FRAMEWORK_PACK_META.pack_version,
    },
  ];
