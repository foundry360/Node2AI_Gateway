-- Final fix: Disable ALL audit triggers, fix user (CORRECTED)
-- This script removes all audit triggers to avoid errors

-- Step 1: Drop ALL audit triggers from all tables (CORRECTED QUERY)
DO $$ 
DECLARE
    r RECORD;
BEGIN
...
SELECT '✅ SUCCESS! User has been synced to the database.' as status;
SELECT 
  u.id,
  u.email,
  u.name,
...
-- Step 7: Show info message
SELECT 'Your user is now synced! Refresh your browser to see your name instead of "Guest".' as next_step;
